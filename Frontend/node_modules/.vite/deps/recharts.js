        ...
        case "string": {
          ...
          if (result === void 0) {
            if (isDeepKey.isDeepKey(path)) {
              return get9(object, toPath.toPath(path), defaultValue);
            } else {
              return defaultValue;
            }
          }
...
var require_use_sync_external_store_with_selector_development = __commonJS({
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-with-selector.development.js"(exports) {
    ...
    (function() {
      ...
      var React74 = require_react(), objectIs = "function" === typeof Object.is ? Object.is : is2, useSyncExternalStore2 = React74.useSyncExternalStore, useRef21 = React74.useRef, useEffect29 = React74.useEffect, useMemo12 = React74.useMemo, useDebugValue2 = React74.useDebugValue;
...
var require_unset = __commonJS({
  "node_modules/es-toolkit/dist/compat/object/unset.js"(exports) {
    ...
    function unsetWithPath(obj, path) {
      const parent = get9.get(obj, path.slice(0, -1), obj);
...
function finalize(rootScope, value, path) {
  ...
  if (!state.finalized_) {
    ...
    each(
      resultEach,
      (key, childValue) => finalizeProperty(rootScope, state, result, key, childValue, path, isSet22)
    );
...
var Rectangle = (rectangleProps) => {
  ...
  var prevXRef = (0, import_react17.useRef)(x);
  var prevYRef = (0, import_react17.useRef)(y);
...
function exp(x, sd) {
  ...
  for (; ; ) {
    ...
    if (digitsToString(t.d).slice(0, wpr) === digitsToString(sum2.d).slice(0, wpr)) {
      while (k--) sum2 = round2(sum2.times(sum2), wpr);
      Ctor.precision = pr;
      return sd == null ? (external = true, round2(sum2, pr)) : sum2;
    }
    sum2 = t;
  }
}
...
function ln(y, sd) {
  ...
  sum2 = numerator = x = divide(x.minus(ONE), x.plus(ONE), wpr);
...
var curry2 = (fn) => curryN(fn.length, fn);
...
var interpolateNumber2 = curry2((a, b, t) => {
  var newA = +a;
  var newB = +b;
  return newA + t * (newB - newA);
});
var uninterpolateNumber = curry2((a, b, x) => {
  var diff = b - +a;
  diff = diff || Infinity;
  return (x - a) / diff;
});
var uninterpolateTruncation = curry2((a, b, x) => {
  var diff = b - +a;
  diff = diff || Infinity;
...
var getTickOfSingleValue = (value, tickCount, allowDecimals) => {
  ...
  var fn = compose2(map((n) => middle.add(new decimal_default(n - middleIndex).mul(step)).toNumber()), range2);
...
var selectOrderedTooltipTicks = createSelector(selectTooltipAxisTicks, (ticks) => (0, import_sortBy5.default)(ticks, (o) => o.coordinate));
...
var calculateWordsByLines = (_ref2, initialWordsWithComputedWith, spaceWidth, lineWidth, scaleToFit) => {
  ...
  var findLongestLine = (words2) => words2.reduce((a, b) => a.width > b.width ? a : b);
...
var ConcentricCircle = (props) => {
  ...
  var concentricCircleProps = _objectSpread25({
    stroke: "#ccc",
    fill: "none"
  }, svgPropertiesNoEvents(props));
  return (
...
var ConcentricPolygon = (props) => {
  ...
  var concentricPolygonProps = _objectSpread25({
    stroke: "#ccc",
    fill: "none"
  }, svgPropertiesNoEvents(props));
...
var getViewBox = (angle, cx, cy, ticks) => {
  var maxRadiusTick = (0, import_maxBy2.default)(ticks, (entry) => entry.coordinate || 0);
  var minRadiusTick = (0, import_minBy2.default)(ticks, (entry) => entry.coordinate || 0);
  return {
    cx,
    cy,
    startAngle: angle,
...
var PolarRadiusAxisWrapper = (defaultsAndInputs) => {
  ...
  }, axisLine && renderAxisLine(props, ticks), tick && renderTicks(props, ticks), React23.createElement(PolarLabelContextProvider, getViewBox(props.angle, props.cx, props.cy, ticks), React23.createElement(PolarLabelFromLabelProp, {
    label: props.label
  }), props.children));
};
...
var Trapezoid = (outsideProps) => {
  ...
  var prevXRef = (0, import_react29.useRef)(x);
  var prevYRef = (0, import_react29.useRef)(y);
  ...
  if (isRange) {
    points.forEach((point2) => {
      if (Array.isArray(point2.value)) {
        ...
        baseLinePoints.push(_objectSpread35(_objectSpread35({}, point2), {}, {
          radius
...
function typeguardBarRectangleProps(_ref2, props) {
  ...
  return _objectSpread37(_objectSpread37(_objectSpread37(_objectSpread37(_objectSpread37({}, props), option), x ? {
    x
  } : {}), y ? {
    y
  } : {}), {}, {
...
function BarBackground(props) {
  ...
  return React36.createElement(React36.Fragment, null, data.map((entry, i) => {
    ...
    var barRectangleProps = _objectSpread38(_objectSpread38(_objectSpread38(_objectSpread38(_objectSpread38({
...
function computeBarRectangles(_ref4) {
  ...
  return displayedData.map((entry, index) => {
    ...
    var barRectangleItem = _objectSpread38(_objectSpread38({}, entry), {}, {
      x,
      y,
      width,
      height,
...
var createScale = (_ref7) => {
  ...
  var len = data.length;
  var scale = point().domain((0, import_range3.default)(0, len)).range([x, x + width - travellerWidth]);
...
function getTicksStart(sign, boundaries, getTickSize, ticks, minTickGap, preserveEnd) {
  ...
  if (preserveEnd) {
    ...
    var tailGap = sign * (tail2.coordinate + sign * tailSize / 2 - end);
    result[len - 1] = tail2 = _objectSpread48(_objectSpread48({}, tail2), {}, {
      tickCoord: tailGap > 0 ? tail2.coordinate - tailGap * sign : tail2.coordinate
    });
    var isTailShow = isVisible(sign, tail2.tickCoord, () => tailSize, start, end);
    if (isTailShow) {
      end = tail2.tickCoord - sign * (tailSize / 2 + minTickGap);
...
function AxisLine2(axisLineProps) {
  ...
  var props = _objectSpread49(_objectSpread49(_objectSpread49({}, otherSvgProps), filterProps(axisLine, false)), {}, {
    fill: "none"
  });
...
function getTickLineCoord2(data, x, y, width, height, orientation, tickSize, mirror, tickMargin) {
...
function Ticks2(props) {
  ...
  var items = finalTicks.map((entry, i) => {
    var {
      line: lineCoord,
      tick: tickCoord
    } = getTickLineCoord2(entry, x, y, width, height, orientation, tickSize, mirror, tickMargin);
...
function CartesianGrid(props) {
  ...
  var chartHeight = useChartHeight();
  var offset = useOffsetInternal();
  var propsIncludingDefaults = _objectSpread50(_objectSpread50({}, resolveDefaultProps(props, defaultProps4)), {}, {
...
var TreemapWithState = class extends import_react70.PureComponent {
  ...
  renderNode(root, node) {
    ...
    var nodeProps = _objectSpread56(_objectSpread56(_objectSpread56({}, svgPropertiesNoEvents(this.props)), node), {}, {
      root
    });
...
var updateYOfTree = (depthTree, height, nodePadding, links) => {
  ...
  return links.map((link) => _objectSpread57(_objectSpread57({}, link), {}, {
...
var sankeyPayloadSearcher = (_, activeIndex, computedData, nameKey) => {
  ...
  var item = (0, import_get7.default)(computedData, "".concat(targetType, "s[").concat(index, "]"));
  if (item) {
...
var buildLinkProps = (_ref3) => {
  ...
  var linkProps = _objectSpread57({
    ...
    linkWidth,
    index: i,
    payload: _objectSpread57(_objectSpread57({}, link), {}, {
      source: sourceNode,
      target: targetNode
    })
...
var SunburstChartImpl = (_ref2) => {
  ...
  function drawArcs(childNodes, options3) {
    ...
    childNodes.forEach((d, i) => {
      ...
      var nodeWithIndex = _objectSpread58(_objectSpread58({}, d), {}, {
        tooltipIndex: currentTooltipIndex
      });
...
var selectFunnelTrapezoids = createSelector([selectChartOffsetInternal, pickFunnelSettings, selectChartDataAndAlwaysIgnoreIndexes], (offset, _ref2, _ref22) => {
  ...
  if (displayedData && displayedData.length) {
    displayedData = displayedData.map((entry, index) => _objectSpread60(_objectSpread60(_objectSpread60({
      payload: entry
...
function FunnelTrapezoids(props) {
  ...
  return React72.createElement(React72.Fragment, null, trapezoids.map((entry, i) => {
    ...
    var trapezoidProps = _objectSpread61(_objectSpread61({}, entry), {}, {
